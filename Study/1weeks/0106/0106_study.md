...
- 실습 화면
    ...
    ![](https://velog.velcdn.com/images/leekh010502/post/4e62486a-c837-40a2-8548-cb4fb4a51fb3/image.png)
    ...
    - merge 후 main 브랜치
      ...
      - 병합 후에 main 브랜치에서 push해줘야합니다
      
  	![업로드중..](blob:https://velog.io/aca47f7c-2840-4130-92b5-720275f41de7)
...
**PR(Pull Request)**은 내가 작업한 브랜치의 변경 사항을 다른 브랜치(보통 main)에 합쳐달라고 공식적으로 요청하는 단계입니다. 협업 프로젝트에서 가장 핵심적인 소통 도구입니다.

1. PR의 의미
**"내가 이만큼 수정했는데, 검토해보고 우리 프로젝트에 합쳐줄래?"**라고 제안하는 것입니다.

직접 merge를 실행하기 전에 팀원들에게 코드의 품질을 확인받고 토론하는 과정을 거칩니다.
...
2. PR의 주요 흐름 (Workflow)
브랜치 작업: feature 브랜치를 생성하여 기능을 구현합니다.

푸시(Push): 로컬에서 작업한 내용을 원격 저장소(GitHub 등)에 올립니다.

PR 생성: GitHub 페이지에서 "Compare & pull request" 버튼을 눌러 PR을 작성합니다.

코드 리뷰: 팀원들이 코드를 보고 질문을 남기거나 수정 요청(Request changes)을 합니다.

병합(Merge): 리뷰가 완료되고 승인(Approve)되면, 관리자가 최종적으로 merge 버튼을 누릅니다.

3. PR을 왜 사용하나요?
코드 품질 유지: 동료의 눈을 통해 버그를 미리 발견하고 더 나은 코드를 작성하게 됩니다.

변경 이력 기록: 왜 이 코드를 수정했는지 설명(Description)을 남기므로 나중에 히스토리를 파악하기 좋습니다.

충돌 방지: main 브랜치에 직접 커밋하는 위험을 줄이고 안정성을 높입니다.
